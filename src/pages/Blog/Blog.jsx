...
export const Blog = () => {
	...
	useEffect(() => {
		Request(
			`https://newsapi.org/v2/${cat === 'none' ? 'everything' : 'top-headlines'}?q=${
				search === '' ? 'news' : search
			}${
				cat !== 'none' ? '&category=' + cat : ''
			}&page=${blogPage}&pageSize=${pageSize}&sortBy=${sort}&apiKey=${API_KEY}`
	...
	return (
		<>
			<PagesHeader title='Our Food Blog' preTitle='ORGANIC INFORMATION' />
			<div className='page-blog'>
				<div className='wrapper'>
					<div className='page-blog__search-container'>
						<div className='page-blog__search-inner'>
							<FontAwesomeIcon icon={faSearch} />
							<input type='text' onChange={searchHeandler} placeholder='Search in Blog' />
						</div>
					</div>
					<div className='page-blog__buttons-container'>
						<ul className='page-blog__buttons'>
							<li
								className={
									cat === 'none'
										? 'page-blog__buttons-item page-blog__buttons-item--active'
										: 'page-blog__buttons-item'
								}
								onClick={changeCat}>
								All
							</li>
